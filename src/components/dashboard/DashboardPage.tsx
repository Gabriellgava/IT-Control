  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-6">
          <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Por Fornecedor</h3>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={stats.distribuicaoFornecedor} dataKey="quantidade" nameKey="nome" cx="50%" cy="50%" outerRadius={80} innerRadius={40}>
                {stats.distribuicaoFornecedor.map((_, i) => <Cell key={i} fill={CORES[i % CORES.length]} />)}
              </Pie>
              <Tooltip />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6">
          ...
          {stats.topAtivos.length === 0 ? <p className="text-sm text-gray-400">Nenhuma saída registrada</p> : (
                ...
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full mt-1">
                      <div className="h-1.5 bg-blue-500 rounded-full" style={{ width: `${Math.min(100, (a.totalSaida / (stats.topAtivos[0]?.totalSaida || 1)) * 100)}%` }} />
                    </div>
                  </div>
