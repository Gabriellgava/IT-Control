...
export function AtivoForm({ ativo, fornecedores, categorias, onSuccess, onCancel }: {
  ativo?: Ativo; fornecedores: Fornecedor[]; categorias: Categoria[];
  ...
  const [form, setForm] = useState({
    ...
    fornecedorId: ativo?.fornecedorId ?? '', categoriaId: ativo?.categoriaId ?? '',
    linkCompra: ativo?.linkCompra ?? '', valorUnitario: ativo?.valorUnitario?.toString() ?? '',
  ...
  const salvar = async () => {
    ...
    try {
      const res = await fetch(ativo ? `/api/ativos/${ativo.id}` : '/api/ativos', {
        ...
        body: JSON.stringify({ ...form, usuarioId: session?.user.id, responsavel: session?.user.name ?? session?.user.email }),
      })
