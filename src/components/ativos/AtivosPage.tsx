...
import type { Ativo, Fornecedor, Categoria } from '@/types'
...
export function AtivosPage() {
  ...
  }, [busca, filtroFornecedor, filtroCategoria, filtroBaixo])
  ...
  // Mapa de estoque total por categoria para exibir status
  const estoquePorCategoria = categorias.reduce((acc, c) => {
    const totalCat = ativos.filter(a => a.categoriaId === c.id).reduce((s, a) => s + a.quantidade, 0)
    acc[c.id] = { total: totalCat, minimo: c.estoqueMinimo, baixo: c.estoqueMinimo > 0 && totalCat <= c.estoqueMinimo }
    return acc
  }, {} as Record<string, { total: number; minimo: number; baixo: boolean }>)

  const exportar = () => exportarCSV(ativos.map(a => ({
    Nome: a.nome, Código: a.codigo, Etiqueta: a.etiqueta ?? '',
    Categoria: a.categoria?.nome ?? '', Fornecedor: a.fornecedor?.nome ?? '',
    'Valor Unitário': a.valorUnitario, Quantidade: a.quantidade,
    'Status Categoria': a.categoriaId && estoquePorCategoria[a.categoriaId]?.baixo ? 'Categoria em estoque baixo' : 'OK',
    'Data Cadastro': formatData(a.criadoEm),
  })), 'ativos-ti')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
        ...
        <Select value={filtroCategoria} onChange={e => setFiltroCategoria(e.target.value)} className="sm:w-44">
          <option value="">Todas categorias</option>
          {categorias.map(c => <option key={c.id} value={c.id}>{c.nome}</option>)}
        </Select>
        <Select value={filtroFornecedor} onChange={e => setFiltroFornecedor(e.target.value)} className="sm:w-44">
      ...
      <Table headers={['Produto', 'Código / Etiqueta', 'Categoria', 'Fornecedor', 'Valor Unit.', 'Qtd', 'Status', 'Ações']} empty={paginados.length === 0}>
        {paginados.map(a => {
          const cat = a.categoriaId ? estoquePorCategoria[a.categoriaId] : null
          const baixo = cat?.baixo ?? false
          return (
            <tr key={a.id} className={`transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50 ${baixo ? 'bg-red-50/50 dark:bg-red-900/5' : ''}`}>
              ...
              <td className="px-4 py-3">
                {a.categoria ? (
                  <div>
                    <Badge variant={cat?.baixo ? 'danger' : 'info'}>{a.categoria.nome}</Badge>
                    <p className="text-xs text-gray-400 mt-1">Total cat.: {cat?.total ?? 0} / mín {a.categoria.estoqueMinimo}</p>
                  </div>
              ...
              <td className="px-4 py-3">{baixo ? <Badge variant="danger"><AlertTriangle className="w-3 h-3 mr-1 inline" />Baixo</Badge> : <Badge variant="success">OK</Badge>}</td>
              <td className="px-4 py-3">
      ...
      {editando && <Modal open={!!editando} onClose={() => setEditando(null)} title="Editar Ativo"><AtivoForm ativo={editando} fornecedores={fornecedores} categorias={categorias} onSuccess={() => { setEditando(null); buscarAtivos() }} onCancel={() => setEditando(null)} /></Modal>}
      <Modal open={!!deletandoId} onClose={() => setDeletandoId(null)} title="Confirmar exclusão">
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">Tem certeza que deseja excluir este ativo? O histórico será preservado.</p>
